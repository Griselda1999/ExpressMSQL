...
materia.get('/api/materia', (req, res) =>{
  ...
  materia.delete('/api/materia/:id', (req, res) =>{
    ...
    // if(respuestaErrorMateriad.length > 0){
  
    //     res.status(400).json({codigo: 400, mensaje: respuestaErrorMateriad})
    //     return
    //    }
    ...
    connection.query('Delete from tbl_materia WHERE id = ?', [id] , function(err, rows, fields) {
        ...
        let errmensaje = {}
            ...
            if(req.body.id != id){
                //Si existe borrarlo
          errmensaje = {codigo: 400, mensaje: `no se encontro la materia con id: ${id}`}
          res.status(400).json(errmensaje)
          return
           }else{
            res.status(200).json(rows)
